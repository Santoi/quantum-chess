...
QuantumPosition::QuantumPosition(uint8_t x, uint8_t y, double prob_,
                                 ...
                                 prob(prob_), entangled(), chessman(chessman_){
    if (prob > 1 || prob < 0)
...
QuantumPosition::QuantumPosition(const Position & position_, double prob_,
                                 ...
                                 prob(prob_), entangled(std::move(list)),
                                 chessman(chessman_){
    if (prob > 1 || prob < 0)
        throw ChessException("probabilidad invalida");
}
...
bool QuantumPosition::isMyChessman(const Chessman * chessman_) {
