...
// Abstract class that represent a chessman_ or quantum chessman_.
class Chessman {
...
protected:
  ...
  // Calculates squares where chessman_ has to pass to go from one position to
  ...
  // Measure other chessman_ entangled with QuantumPosition.
  ...
  // Loads a vector with all posible moves of the chessman_.
  ...
  // Entangles chessman_ with the other chessman_. Splitting this chessman_ with
...
public:
  ...
  // Moves chessman_ from initial position to final, validating the move.
  virtual bool move(const Position &initial, const Position &final);

  // Splits chessman_ from initial position to final1 and final2 validating.
  virtual void split(const Position &initial, const Position &final1,
                     const Position &final2);

  // Merges chessman_ from initial1 and initial2 to final, validating.
  void merge(const Position &initial1, const Position &initial2,
             const Position &final);

  // Returns true if chessman_ is in quantum state.
  bool isQuantum() const;

  // Returns true is chessman_ is white.
  bool isWhite() const;
  ...
  // Load list with all the positions of chessman_.
  void getAllPositions(std::list<Position> &output) const;

  // Load list with all positions of all entangled chessman_.
  void getEntangledPositions(std::list<Position> &output);

  // Returns character that represents chessman_.
