#include "login_scene.h"

LoginScene::LoginScene(LoginStateHandler &login_state_handler_, Window &window)
    :
    Scene(window),
    login_state_handler(login_state_handler_),
    ip() {

}
//
//void LoginScene::setScale(const int& scale_) {
//    scale = scale_;
//}

void LoginScene::render() {
  login_state_handler.tellRendererWhatToRender(*this);
}

void LoginScene::renderIPAndPortFields(Button &connect_button,
                                       TextEntryButton &ip_text_entry,
                                       TextEntryButton &port_text_entry) {
  connect_button.setAreaAndPosition(200, 300, scale * 0.1, scale * 0.3);
  connect_button.render();
  ip_text_entry.setAreaAndPosition(10, 10, scale * 0.1, scale * 0.8);
  ip_text_entry.render();
  port_text_entry.setAreaAndPosition(10, 100, scale * 0.1, scale * 0.3);
  port_text_entry.render();
}

void LoginScene::renderMatchButtons(
    std::vector<std::unique_ptr<Button>> &match_buttons) {
  for (int i = 0; i < (int) match_buttons.size(); i++) {
    match_buttons[i]->setAreaAndPosition(i * 20 + 10, 200, 0.1 * scale,
                                         0.1 * scale);
    match_buttons[i]->render();
  }
}

void LoginScene::renderConnectedSprite(TextureSprite &texture) {
  texture.render(10, 10, scale * 0.9, scale * 0.9);
}
