...
ClientHandlersReceiver::ClientHandlersReceiver(Socket& socket, const ClientData & client_data_, BlockingQueue<Instruction>&
                                                updates_queue)
                        :client_socket(socket), client_data(client_data_), updates_queue(updates_queue) {
}

ClientHandlersReceiver::ClientHandlersReceiver(ClientHandlersReceiver&& otherReceiver, Socket& socket)
                        :Thread(std::move(otherReceiver)), client_socket(socket), client_data(otherReceiver.client_data),
...
void ClientHandlersReceiver::pushToQueueExitInstruction() {
    std::shared_ptr <Instruction> exit_instruction = std::make_shared<ExitInstruction>(client_data);
...
void ClientHandlersReceiver::run() {
    try{
        ...
        std::cerr << "Client id " << client_data.getId() << " " << error.what() << std::endl;
