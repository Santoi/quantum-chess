...
string que este almacena una vez a sido presionado.
...
la ip y el puerto para poder conectarse al servidor, guardándo el socket creado,
...
conectandose al servidor, puede ser que el usuario proporcione un ip y/o puertos
...
login. Se hace enfásis en que cuando el LoginHandlerThread (el controlador de
...
estado "Conectandose al Servidor".
...
De nuevo se hace enfásis en que no importa el estado actual del login: siempre
...
Ahora bien, cuando un usuario interactua con un sistema informático, es probable
...
sencible al segundo caso: ¿Qué pasa si el usuario coloca un ip y/o puerto
...
procede a explicar *qué* sucede cuándo le piden a un botón/entrada de texto que
se renderice. Para eso, primero se muestra el siguiente diagrama de clases:
...
el que se puede obtener las imagenes correspondientes del botón (una cuando este
ha sido presionado y la otra para cuando no) del repositorio de imagenes de
botones.
