...
ChessExceptionInstruction::ChessExceptionInstruction(const ClientData & instructor_data_, std::string&& message):
...
void ChessExceptionInstruction::makeActionAndNotifyAllListeningQueues(
  ...
  std::shared_ptr<Instruction> this_instruc_ptr = std::make_shared<ChessExceptionInstruction>(instructor_data, std::move(this->message));
  listening_queues.at(instructor_data.getId()).push(this_instruc_ptr);
}

void ChessExceptionInstruction::fillPacketWithInstructionsToSend(ServerProtocol &protocol,
                                                                 Packet &packet,
